...
UI自動化には[Gua](https://github.com/link1345/gua) v1.18.0のGodot GDScriptアドオンを使用しています。実行中は `ws://127.0.0.1:8765` でGua bridgeが待ち受け、標準Godot Controlツリーを自動的に公開します。
...
`.github/workflows/gua-tests.yml` で [`link1345/gua-tester`](https://github.com/link1345/gua-tester) v2.2のGodot Action（`link1345/gua-tester/godot@v2.2`）を使用し、`master` へのpushとpull requestで実際のGodotプロセスを操作するUIテストを実行します。CIではNuGetパッケージと同じGua v1.18.0の公開アドオンをダウンロードするため、DLLをリポジトリに含める必要はありません。

### Visual差分Viewer

pull requestでVisual比較が失敗すると、`visual-report@v2.2`が`comparison.json`とPNGをAstro製の静的Viewerへ変換し、`gua-visual-report`という通常のActions artifactとして保存します。ViewerにはExpected／Diff／Actualの3列表示とExpected／Actual比較スライダーがあります。
